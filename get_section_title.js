...
var rr1=/^[ 　>]*第[0-9一二三四五六七八九十零〇]{1,2}([章节篇辑讲]|(部分))[ 　]{0,30}\n|^[ 　>]*第[0-9一二三四五六七八九十零〇]{1,2}([章节篇辑讲]|(部分))[ 　]+\S{0,30}.{0,30}\n/gm;
...
//这个表达式可以匹配到‘     \n’,但是实际操作中每一行是trim()过了再匹配正则的，所以不会出现这种内容被匹配到
// var rr7=/^\s*(?!\n|\t)*\S{1,20} *\n/gm;
// var rr7=/^[         ]*([^\n\r\v\t\f。]| | ){1,30}(?!。)[  ]*\n(?=\s{0,50}\S{20,})/gm;
// var rr7=/^[  \t]*.{1,40}(?!。)[  ]*\n(?=[ \n]{0,50}\S{20,})/gm; //错误率比起上一条提高了，应该还是一股脑识别了一堆乱七八糟
//下面这条使用了好一段时间，后来发现会匹配到 “。\n”“xxx。\n”，于是做了调整
// var rr7=/^[  \t]*.{1,40}(?!。)[  ]*\n(?=[ \n]{0,50}[^\n\r\f]{20,})/gm;
// var sr7='^[  \\t]*.{1,40}(?!。)[  ]*\\n(?=[ \\n]{0,50}[^\\n\\r\\f]{20,})';
//调整如下（注意.是会排除掉\n的，所以不写.的话要另外排除掉，否则会匹配到"！第十章\n从知识与技能相互转化的观点 寻找语言教学高耗低效的原因\n"
// var rr7=/^[  \t]*[^。]{1,40}(?!。)[  ]*\n(?=[ \n]{0,50}[^\n\r\f]{20,})/gm;
// var sr7='^[  \\t]*[^。]{1,40}(?!。)[  ]*\\n(?=[ \\n]{0,50}[^\\n\\r\\f]{20,})';
var rr7=/^[ 　\t]*[^。\n\r\f]{1,20}(?!。)[ 　]*\n(?=[ \n]{0,50}[^\n\r\f]{40,})/gm;
var sr7='^[ 　\\t]*[^。\n\r\f]{1,20}(?!。)[ 　]*\\n(?=[ \\n]{0,50}[^\\n\\r\\f]{40,})';
...
//有一种情况：'短标题 \n \n 一大段文字'，有的文本会很作死，一段下面都要空一行，这条式子也可以识别这种情况
//有些段落里出现了空格，有可能其前边的标题就无法识别了（已改进）

//判断第一行是否是标题 这里有错，还是会匹配到其他短标题，暂时不这样用，已经专门写了代码专门对其进行识别
//本可以用\A来匹配，但是js不支持\A
// var rr8=/(?<!.)[\S| ]{1,25}[  ]*\n/gm
var rr8=/(?<![.\n])[\S| ]{1,25}[ 　]*\n/gm
var sr8='(?<![.\\n])[\\S| ]{1,25}[ 　]*\\n'
...
//下面这条式子的正确率更高
var allRegExps = sr1+'|'+sr2+'|'+sr3+'|'+sr5+'|'+sr6+'|'+sr7;
// var allRegExps = sr1+'|'+sr2+'|'+sr3+'|'+sr5+'|'+sr6; //不识别短标题
